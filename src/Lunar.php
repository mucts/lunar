...
class Lunar
{
    ...
    public function convertSolarToLunar(int $year, int $month, int $date): array
    {
        ...
        if ($year == self::MIN_YEAR && $month <= 2 && $date < 9 || self::MAX_YEAR < $year) {
            throw new Exception('date can\'t be greater than %s or less than %s', self::MIN_YEAR . '-02-09', self::MAX_YEAR);
        }
    ...
    {
        $yearData = self::LUNAR_INFO[$year - self::MIN_YEAR];
        if ($year == self::MIN_YEAR && $month < 2 || self::MAX_YEAR < $year) {
            throw new Exception('date can\'t be greater than %s or less than %s', self::MIN_YEAR . '-02', self::MAX_YEAR);
        }
        $monthDaysAry = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
        $dd = $monthDaysAry[$month];
    ...
    {
        $monthHash = [31, $this->isLeapYear($year) ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
     ...
     * @return int
     */
    function getLunarMonthDays(int $year, int $month): int
    {
     ...
     * @return array
     */
    function getLunarByBetween(int $year, int $between): array
    {
        $lunarArray = [];
        $t = 0;
        $e = 0;
     ...
     */
    function getCapitalNum(int $num, bool $isMonth)
    {
